...
GameWindow::GameWindow(wstring title, int width, int height, float red, float green, float blue, float alpha):
	Win32OGLWindow(title, width, height)
{
	this->gameEngine = NULL;
	this->WndProcedure = GameWindow::WndProc;
	this->self = this;
	this->background.red = red;
	this->background.green = green;
	this->background.blue = blue;
