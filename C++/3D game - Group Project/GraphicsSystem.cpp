...
void GraphicsSystem::update(float elapsedSeconds)
{
	...
	objectMgr->getPlayer()->update(elapsedSeconds);
	// May be used to update other stuff
	this->theGame->update(elapsedSeconds);

	objectMgr->updateVisibleObjects();
	objectMgr->clearCollisions();
	objectMgr->checkForCollisions();
}
