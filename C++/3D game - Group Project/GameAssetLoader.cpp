...
void GameAssetLoader::loadGameObjects()
{
	...
	while (!this->atEnd() && this->nextLine() != "</gameobjects>") {
		if (this->currLine() == "<axis>") {
			...
			this->loadConstruct();
			this->logger->log("Construct loaded");
		}
		else if (this->currLine() == "<door>") {
			this->loadDoor();
			this->logger->log("Door loaded");
		}
	}
}
...
void GameAssetLoader::loadDoor()
{
	...
	while (!this->atEnd() && this->nextLine() != "</door>") {
		if (this->currLine() == "<name>") {
			this->loadElement(fields, "</name>");
			name = fields[0];
			door = new Door(name);
			...
			this->loadElement(fields, "</position>");
			door->referenceFrame.translateWorld(
				std::stof(fields[0]),
				std::stof(fields[1]),
				std::stof(fields[2]));
		}
			...
			surface = this->loadSurface();
			door->surfaces[surface->getName()] = surface;
		}
		fields.clear();
	}

	objectMgr->addObject(door->getName(), door);
}
