...
void OGLReferenceFrame::scale(float scaleAmount)
{
	glm::mat4 scaleMatrix(1.0f);
	glm::vec3 scaleVector(scaleAmount);

	scaleMatrix[0] *= scaleVector[0];
	scaleMatrix[1] *= scaleVector[1];
	scaleMatrix[2] *= scaleVector[2];

	this->orientation *= scaleMatrix;
}