	...
	ITimer *timer) : BaseObject(logger)
{
	...
	GameWorld* gameWorld = this->graphics->getGameWorld();
	gameWorld->setInputSystem(this->inputSystem);

	gameWorld->setTheGame(this->theGame);

	gameWorld->getObjectManager()->setPlayer(this->player);
